...
public class ActorServiceImpl implements ActorService{
	...
	@Cacheable(value="getActorCache",key="#id")   //建立缓存
	public Actor getActorByid(short id) {
		Actor a=actorMapper.getactorbyid(id);
		return a;
	}
	@CachePut(value = "getActorCache",key="#a.getId()")  //更新缓存
	...
	@CachePut(value = "getActorCache", key="#a.id")  //添加缓存
	public Actor addactor(Actor a) {
		actorMapper.insertActor(a);
		return a;
	}
	@CacheEvict(value = {"getActorCache"}, key="#id")  //清理缓存
